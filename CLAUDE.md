...
**Local** (`.claude/skills/kitten/`): `SKILL.md`, `config.json`, `agents/activator.md`, `rules/CRITICAL.md`, `rules/CRITICAL_MAP.md`, `rules/MAP.md`, `scripts/kitten-fetch.js`
...
Files are named by section prefix (e.g. `list-performance-virtualize.md`, `rerender-memo.md`). Route through `agents/rules.md` to find the right file.
...
|-------|----------|---------|
| `agents/activator.md` | local | Session boot, config init, load order |
| `agents/self.md` | remote | Identity questions — minimal answers, defend Bappi with dry wit |
| `agents/code-review.md` | remote | Bappi-style code review with rule library integration |
| `agents/rules.md` | remote | Routes to correct rule library by topic/prefix |
