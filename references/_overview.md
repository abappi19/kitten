...
| `references/bappi/env-config.md` | Environment Config  | getEnvVars() factory, EXPO*PUBLIC* prefix, per-environment config objects, app.config.ts runtime injection, EAS build profile env, Turbo globalEnv.                                                                    | Setting up env vars, reviewing config access, working with app.config.ts or eas.json.           |
...
| File                            | Title              | Description                                                                                                                                                                                               | When to load                                                                                   |
| ------------------------------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `references/bappi/debugging.md` | Debugging Patterns | Iron Law (root cause first), 3-attempts architecture checkpoint, structured error logging, Reactotron setup, Flash Message for user errors, platform-aware error context, common silent failure patterns. | Debugging, tracing errors, reviewing error handling code, setting up observability.            |
...
| File                                     | Title              | Description                                                                                                                                                                                                                          | When to load                                                                                                                                                   |
| ---------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `references/bappi/creation-lifecycle.md` | Creation Lifecycle | Universal 12-phase execution framework for building anything from scratch — modules, libraries, backends, web apps, packages, design systems. Hard gates at Phases 1–3 (no code before Phase 4). Domain-specific guidance per phase. | Any creation/greenfield task — "create a module", "build a library", "make a package", "design a design system". Routed by the planner's classification table. |
...
| `references/kitten/execution-contract.md`        | Execution Contract            | Universal enforcement framework for all multi-step agent flows — execution rules, self-validation checklist, phase gates, output schemas, anti-pattern guards, execution modes (Strict/Fast Track/Expert). Enforced by CX_R17.                                           | Loaded automatically by any agent with a sequential flow. Not fetched manually — agents reference it in their headers. |
| `references/kitten/planning-directory.md`        | Planning Directory Convention | Canonical `.planning/` layout — minimal index `PLAN.md`, per-initiative folders with their own `PLAN.md` + `phases/`, small-initiative escape hatch, lifecycle (create → in-progress → done → archive), migration prompt for non-conforming folders. Enforced by CX_R18. | Every Feature Plan, Creation Lifecycle run, project bootstrap, and refinement spec — persist plans per the convention. |
...
| Expo API routes / EAS Hosting                     | `bappi/nextjs.md`                                          |
| Expo SDK upgrade / deprecations                   | `bappi/tooling.md`                                         |
...
| Create module / library / package                 | `bappi/creation-lifecycle.md`                              |
| Build a backend service                           | `bappi/creation-lifecycle.md`                              |
| Design a design system                            | `bappi/creation-lifecycle.md`                              |
