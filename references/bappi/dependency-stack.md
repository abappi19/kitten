...
- `eslint.config.mjs` — flat config: `@eslint/js` + `typescript-eslint` + `eslint-plugin-prettier/recommended`
- `prettier.config.mjs` — singleQuote, trailingComma all, arrowParens avoid, endOfLine auto, tabWidth **4**
- `lint-staged.config.mjs` — covers `.{js,jsx,ts,tsx}` (eslint + prettier) and `.{json,md,css,scss}` (prettier only)
- `pre-commit` husky hook — `#!/usr/bin/env sh`, `set -e`, `FORCE_COLOR=0`, `NO_COLOR=1`, then `bunx lint-staged`
- `package.json` scripts — `lint`, `lint:fix`, `format`, `format:check`, `type-check`, `verify`
- `engines` field — enforce bun: `"npm": "please_use_bun"` etc.
- `.vscode/settings.json` + `.vscode/extensions.json`

For monorepos: tooling deps at root, scripts cover all `packages/*/src`.
Full config details: `references/bappi/tooling.md`.
