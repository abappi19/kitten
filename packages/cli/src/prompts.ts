...
export async function askLanguage(): Promise<CommLang> {
    ...
    while (true) {
        const input = (await ask('  Preferred language [English / Bangla]: '))
            .trim()
            .toLowerCase();
        if (input === 'english' || input === 'en') return 'English';
        if (input === 'bangla' || input === 'bn' || input === 'বাংলা')
            return 'Bangla';
...
export async function askToken(): Promise<string> {
    ...
    while (true) {
        const token = (
            await askSecret('  GitHub Personal Access Token: ')
        ).trim();
...
export async function askBranch(): Promise<Branch> {
    ...
    while (true) {
        const input = ((await ask('  Branch [main]: ')).trim() ||
            'main') as Branch;
