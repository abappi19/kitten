...
function copyDir(src: string, dest: string): void {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      console.log(`  \x1b[32m✓\x1b[0m ${path.relative(SKILL_ROOT, srcPath)}`);
    }
  }
}
...
function patchKLoad(token: string): void {
  ...
  if (!src.includes(KLOAD_AUTH_PLACEHOLDER)) {
    throw new Error('k_load.py _AUTH_V placeholder not found — patch target may have changed');
  }
  src = src.replace(KLOAD_AUTH_PLACEHOLDER, `_AUTH_V    = '${encoded}'`);
  fs.writeFileSync(kloadPath, src, 'utf-8');
  console.log('  \x1b[32m✓\x1b[0m scripts/k_load.py patched with build-time token');
}
